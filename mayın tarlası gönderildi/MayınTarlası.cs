...
{
    internal class MayınTarlası
    {
        Size buyukluk_;
        List<Mayin> mayinlar; // mayınların listesi
        int mayınlıGridSayısı;
        Random rdm = new Random(); // atamaları random yapar

        public MayınTarlası(Size buyukluk, int mayin_sayisi)
        {
            mayinlar = new List<Mayin>();
            mayınlıGridSayısı = mayin_sayisi;
            buyukluk_ = buyukluk;
            ...
            mayinlari_doldur();
            this.mayınlıGridSayısı = mayınlıGridSayısı;
        }
        ...
        // random atanan noktalara mayınları yerleştirir
        public void mayinlari_doldur()
        {
            int sayi = 0;
            while (sayi < mayınlıGridSayısı)
            {
                int i = rdm.Next(0, mayinlar.Count);
                Mayin item = mayinlar[i];
                if (item.istherebomb == false)
                {
                    item.istherebomb = true; // konumda mayın olduğu bilgisini girer
                    sayi++;
                }
            }
        }
